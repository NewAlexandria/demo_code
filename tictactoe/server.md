...
For the sake of some future expansion I have included a ‘flat fileʼ as the read/write point for tokens. A database is not commonly accessed from a proxy router (eg Nginx), due to norms, though some memory stores could be.  We could make it access a DB/store, but reading a config file is a more standard practice, that also lets usdraw the file from any mappable filesystem. 

The config file can maintain the tokens, and get read in as an in-memory store, to minimize the routing delay to less that 10ms.  This also allows for a failsafe to read directly from disk if an issue occurs.  When the size of active sessions sessions becomes large enough to force sharding, then anycasting DNS can be used to enable to the proxy cluster to remain scalable. If (when) we get to a scale of global routing, and we have sharded routers, we can replicate the proxy routers within reasonable zones for the geographies of our engagement.
...
The game server can be written first, in a lean way to garner beta testing.
